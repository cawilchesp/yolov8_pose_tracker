...
class AnnotationSink:
    ...
    def on_detections(self, detections: sv.Detections, key_points: sv.KeyPoints, image: np.array) -> np.array:
        ...
        if self.vertex:
            annotated_image = self.vertex_annotator.annotate(
                scene=annotated_image,
        ...
        # Draw keypoint edges
        if self.edge:
            annotated_image = self.edge_annotator.annotate(
                scene=annotated_image,
                key_points=key_points
            )
        ...
        if self.vertex_label:
            annotated_image = self.vertex_label_annotator.annotate(
                scene=annotated_image,
                key_points=key_points
            )
